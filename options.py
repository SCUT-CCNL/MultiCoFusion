...
def my_parse_args_pathomic(): #For MultiCoFusion
    ...
    parser.add_argument('--exp_name', type=str, default='my_surv_grade', help='name of the project. It decides where to store samples and models')
    parser.add_argument('--gpu_ids', type=str, default='0', help='gpu ids: e.g. 0  0,1,2, 0,2. use -1 for CPU')
    parser.add_argument('--mode', type=str, default='pathomic', help='mode')
    parser.add_argument('--model_name', type=str, default='pathomic_resnet_sgcn_surv_grad_JT', help='mode')
    parser.add_argument('--task', type=str, default='surv_grad', help='surv | grad | surv_grad')
    parser.add_argument('--act_type_1', type=str, default='Sigmoid', help='activation function') ##输出的
    parser.add_argument('--act_type_2', type=str, default='LSM', help='activation function') ##输出的
    parser.add_argument('--input_size_omic', type=int, default=10673, help="input_size for omic vector")
    parser.add_argument('--input_size_path', type=int, default=512, help="input_size for path images")
    parser.add_argument('--init_gain', type=float, default=0.02, help='scaling factor for normal, xavier and orthogonal.')
    parser.add_argument('--label_dim_1', type=int, default=1, help='size of output')
    parser.add_argument('--label_dim_2', type=int, default=3, help='size of output')
    ...
    parser.add_argument('--finetune', default=1, type=int, help='5e-4 for Adam | 1e-3 for AdaBound') ###对单omic没影响
    ...
    parser.add_argument('--weight_decay', default=0.0004, type=float, help='Used for Adam. L2 Regularization on weights. I normally turn this off if I am using L1. You should try')
...
def parse_args_omic():
    ...
    parser.add_argument('--act_type', type=str, default='Sigmoid', help='activation function') ##输出的
    parser.add_argument('--input_size_omic', type=int, default=320, help="input_size for omic vector")
    parser.add_argument('--input_size_path', type=int, default=512, help="input_size for path images")
    parser.add_argument('--init_gain', type=float, default=0.02, help='scaling factor for normal, xavier and orthogonal.')
    ...
    parser.add_argument('--lambda_reg', type=float, default=0.0003)
    parser.add_argument('--lambda_nll', type=float, default=1)
    parser.add_argument('--fusion_type', type=str, default="pofusion_A", help='concat | pofusion')
    parser.add_argument('--skip', type=int, default=0)
    parser.add_argument('--use_bilinear', type=int, default=1)
    ...
    parser.add_argument('--weight_decay', default=0.0005, type=float, help='Used for Adam. L2 Regularization on weights. I normally turn this off if I am using L1. You should try')
...
def parse_args_path():
    ...
    parser.add_argument('--beta2', type=float, default=0.999, help='0.9, 0.5 | 0.25 | 0')
    parser.add_argument('--lr_policy', default='linear', type=str, help='5e-4 for Adam | 1e-3 for AdaBound')
    parser.add_argument('--finetune', default=0, type=int, help='5e-4 for Adam | 1e-3 for AdaBound') ###对单omic没影响
    parser.add_argument('--final_lr', default=0.1, type=float, help='Used for AdaBound')
    parser.add_argument('--reg_type', default='none', type=str, help="regularization type")
    parser.add_argument('--niter', type=int, default=0, help='# of iter at starting learning rate')
    parser.add_argument('--niter_decay', type=int, default=50, help='# of iter to linearly decay learning rate to zero')
    parser.add_argument('--epoch_count', type=int, default=1, help='start of epoch')
    parser.add_argument('--batch_size', type=int, default=8, help="Number of batches to train/test for. Default: 256")
    parser.add_argument('--lambda_cox', type=float, default=1)
...
def parse_args_pathomic():
    ...
    parser.add_argument('--gpu_ids', type=str, default='0', help='gpu ids: e.g. 0  0,1,2, 0,2. use -1 for CPU')
    parser.add_argument('--mode', type=str, default='pathomic', help='mode')
    parser.add_argument('--model_name', type=str, default='pathomic', help='mode')
    parser.add_argument('--use_vgg_features', type=int, default=0, help='Use pretrained embeddings')
...
def parse_args_omic_grade():
    ...
    parser.add_argument('--useRNA', type=int, default=0) # Doesn't work at the moment...:(
    parser.add_argument('--useSN', type=int, default=1)
    parser.add_argument('--act_type', type=str, default='LSM', help='activation function') ##输出的
    ...
    parser.add_argument('--lambda_reg', type=float, default=0.0003)
    parser.add_argument('--lambda_nll', type=float, default=1)
    parser.add_argument('--fusion_type', type=str, default="pofusion_B", help='concat | pofusion')
...
def parse_args_pathomic_grade():
    ...
    parser.add_argument('--path_scale', type=int, default=1)
    parser.add_argument('--grph_scale', type=int, default=1)
    parser.add_argument('--omic_scale', type=int, default=2)
    parser.add_argument('--mmhid', type=int, default=64)
    parser.add_argument('--init_type', type=str, default='none', help='network initialization [normal | xavier | kaiming | orthogonal | max]. Max seems to work well')
    parser.add_argument('--dropout_rate', default=0.25, type=float, help='0 - 0.25. Increasing dropout_rate helps overfitting. Some people have gone as high as 0.5. You can try adding more regularization')
    parser.add_argument('--use_edges', default=1, type=float, help='Using edge_attr')
    ...
    return opt
######################
### multimodalprognosis surv ###
######################
def my_parse_args_multimodalprognosis():
    parser = argparse.ArgumentParser()
    parser.add_argument('--dataroot', default='./data/TCGA_GBMLGG', help="datasets")
    parser.add_argument('--checkpoints_dir', type=str, default='./checkpoints/TCGA_GBMLGG', help='models are saved here')
    parser.add_argument('--exp_name', type=str, default='my_surv_grade', help='name of the project. It decides where to store samples and models')
    parser.add_argument('--gpu_ids', type=str, default='0,1', help='gpu ids: e.g. 0  0,1,2, 0,2. use -1 for CPU')
    parser.add_argument('--mode', type=str, default='multimodalprognosis', help='mode')
    parser.add_argument('--model_name', type=str, default='multimodalprognosis_grad', help='mode')
    parser.add_argument('--task', type=str, default='grad', help='surv | grad | surv_grad')
    parser.add_argument('--act_type_1', type=str, default='none', help='activation function') ##输出的
    parser.add_argument('--act_type_2', type=str, default='none', help='activation function') ##输出的
    parser.add_argument('--input_size_omic', type=int, default=10673, help="input_size for omic vector")
    parser.add_argument('--input_size_path', type=int, default=512, help="input_size for path images")
    parser.add_argument('--init_gain', type=float, default=0.02, help='scaling factor for normal, xavier and orthogonal.')
    parser.add_argument('--label_dim_1', type=int, default=1, help='size of output')
