...
def regularize_MM_omic(model, reg_type=None):
    ...
    # ### by KT
    # l1_reg = None
    # for W in model.module.parameters():
    #     if l1_reg is None:
    #         l1_reg = torch.abs(W).sum()
    #     else:
    #         l1_reg = l1_reg + torch.abs(W).sum() # torch.abs(W).sum() is equivalent to W.norm(1)
...
def poolGradeTestPD(ckpt_name='./checkpoints/', model='pathgraphomic_fusion', split='test'):
    ...
    for k in range(1, 16):
        ...
        # model_2 = "pathomic_resnet_sgcn_grad_2"
        # pred_2 = pickle.load(open(ckpt_name + '/%s/%s_%d%spred_%s.pkl' % (model_2, model_2, k, use_patch, split), 'rb'))
        # pred[3] = pred_2[3]

        if 'cox' not in model:
            pred[3] = np.argmax(pred[3], axis=1)
            surv_all = pd.DataFrame(np.stack(np.array(pred))).T
            surv_all.columns = ['Hazard', 'Survival months', 'censored', "Grade_P", 'Grade']
    return surv_all


def poolSurvTestPD(ckpt_name='./checkpoints/', model='pathgraphomic_fusion', split='test', zscore=False, agg_type='Hazard_mean'):
    ...
    for k in range(1,16):
        ...
        if 'cox' not in model:
            ...
            data_cv = pickle.load(open('./data/TCGA_GBMLGG/splits/gbmlgg15cv_all_st_1_1_0_rnaseq_new.pkl', 'rb'))
...
def getAggHazardCV(ckpt_name='./checkpoints/', model='pathgraphomic_fusion', split='test',
                   agg_type='Hazard_mean'):
    result = []

    use_patch, roi_dir, use_vgg_features = '_', 'all_st', 1
    ...
    for k in range(1, 16):
        ...
        assert np.all(data_cv_split_k[split]['t'] == pred[1])  # Data is correctly registered
        all_dataset = data_cv['data_pd'].drop('TCGA ID', axis=1)
        # all_dataset = all_dataset.drop(labels='TCGA-06-0152', axis=0) ### 去除没用到的样本'TCGA-06-0152'
        all_dataset_regstrd = all_dataset.loc[data_cv_split_k[split]['x_patname']]  # Subset of "all_datasets" (metadata) that is registered with "pred" (predictions)
        assert np.all(np.array(all_dataset_regstrd['Survival months']) == pred[1])
        assert np.all(np.array(all_dataset_regstrd['censored']) == pred[2])
        assert np.all(np.array(all_dataset_regstrd['Grade']) == pred[4])
...
def calcGradMetrics(ckpt_name='./checkpoints/', model='pathgraphomic_fusion', split='test', avg='micro'):
    auc_all = []
    ap_all = []
    f1_all = []
    f1_gradeIV_all = []
...
# Plot Utils
################
def makeKaplanMeierPlot(ckpt_name='./checkpoints/', model='omic', split='test', zscore=False, agg_type='Hazard_mean'):
...
def makeHazardSwarmPlot(ckpt_name='./checkpoints/', model='path', split='test', zscore=True, agg_type='Hazard_mean'):
    mpl.rcParams['font.family'] = "arial"
    # data = poolSurvTestPD(ckpt_name=ckpt_name, model=model, split=split, zscore=zscore, agg_type=agg_type)
    data = poolGradeTestPD(ckpt_name=ckpt_name, model=model, split=split, zscore=zscore, agg_type=agg_type)
    ...
    data['Grade_P'] = data['Grade_P'].astype(int).astype(str)
    data['Grade_P'] = data['Grade_P'].str.replace('0', 'Predicted Grade II', regex=False)
    data['Grade_P'] = data['Grade_P'].str.replace('1', 'Predicted Grade III', regex=False)
    data['Grade_P'] = data['Grade_P'].str.replace('2', 'Predicted Grade IV', regex=False)
    ...
    sns.swarmplot(x = 'Grade', y='Survival months', data=data, hue='Grade_P',
                  palette={"Predicted Grade II":"#AFD275" , "Predicted Grade III":"#7395AE", "Predicted Grade IV":"#E7717D"},
...
def makeHazardBoxPlot(ckpt_name='./checkpoints/', model='omic', split='test', zscore=True, agg_type='Hazard_mean'):
    mpl.rcParams['font.family'] = "arial"
...
def makeAUROCPlot(ckpt_name='./checkpoints/', model_list=['path', 'omic', 'pathomic'], split='test', avg='micro', use_zoom=False):
    mpl.rcParams['font.family'] = "arial"
    # colors = {'path_resnet152_grad':'dodgerblue', 'omic_sgcn_grade':'orange', 'path_resnet152_surv_grad':'green', 'omic_sgcn_surv_grad_DWA':'cyan', 'pathomic_resnet_sgcn_grad_2':'black', 'pathomic_resnet_sgcn_surv_grade_4_DWA':'red'}
    # names = {'path_resnet152_grad':'SS Resnet', 'omic_sgcn_grade':'SS SGCN', 'path_resnet152_surv_grad':'SM Resnet', 'omic_sgcn_surv_grad_DWA':'SM_DWA SGCN', 'pathomic_resnet_sgcn_grad_2':'MS Resnet+SGCN', 'pathomic_resnet_sgcn_surv_grade_4_DWA':'MM_DWA Resnet+SGCN'}
    colors = {'pathomic_resnet_sgcn_surv_grad_JT':'red', 'pathomic_resnet_sgcn_grad_2':'green', 'omic_sgcn_surv_grade_JT':'blue', 'path_resnet152_surv_grad_JT':'cyan', 'x':'black', 'x':'red'}
    names = {'pathomic_resnet_sgcn_surv_grad_JT':'MMMT ResNet152+SGCN', 'pathomic_resnet_sgcn_grad_2':'MMST ResNet152+SGCN', 'omic_sgcn_surv_grade_JT':'SMMT SGCN_AT', 'path_resnet152_surv_grad_JT':'SMMT ResNet152_AT', 'x':'MS Resnet+SGCN', 'x':'MM_DWA Resnet+SGCN'}
    ...
    for i in classes:
        ...
        for m, model in enumerate(model_list):
            ...
            for k in range(1,16):
                ...
                cfm += confusion_matrix(grade, grade_pred.argmax(axis=1))
            ...
            print('\t'+'%s - AUC: %0.4f ± %0.4f' % (model, mean_auc, std_auc))
            
            if use_zoom:
                alpha, lw = (0.8, 6) if model =='pathgraphomic_B' else (0.5, 6)
                ...
                alpha, lw = (0.8, 4) if model =='pathgraphomic_B' else (0.5, 3)
                plt.plot(mean_fpr, mean_tpr, color=colors[model],
                     label=r'%s (AUC = %0.3f $\pm$ %0.3f)' % (names[model], mean_auc, std_auc), lw=lw, alpha=alpha)
                std_tpr = np.std(tprs, axis=0)
                tprs_upper = np.minimum(mean_tpr + std_tpr, 1)
                ...
                axi.legend(loc="lower right", prop={'size': 15})
                axi.tick_params(axis='both', which='major', labelsize=30)
                axi.set_xlabel('False Positive Rate', size=20)
                axi.set_ylabel('True Positive Rate', size=20)
                # axi.title("micro-AUC", size=20)
