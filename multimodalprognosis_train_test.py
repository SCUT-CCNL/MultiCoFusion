...
def train(opt, data, device, k, graph):
    ...
    for epoch in tqdm(range(opt.epoch_count, opt.niter+opt.niter_decay+1)):
        ...
        for batch_idx, (x_path, x_grph, x_omic, censor, survtime, grade, clin, index) in enumerate(train_loader):#
            ...
            r_omic, r_path, pred_surv = model(x_path=x_path, x_grph=x_grph, x_omic=x_omic, gene_adj=graph, clin=clin)
            loss_cox = CoxLoss(survtime, censor, pred_surv, device)
            loss_sim = 0
            len_sample = r_omic.shape[0]
            # sim_x_x = torch.sum(r_omic * r_path, dim=1) / (torch.sum(r_omic, dim=1) * torch.sum(r_path, dim=1))
            for s1 in range(len_sample):
                sim_all_s1 = torch.sum(r_omic[s1,:]*r_path, dim=1) / (torch.sum(r_omic[s1,:]) * torch.sum(r_path, dim=1))
                sim_x_x = sim_all_s1[0]
                sim_x_y = sim_all_s1[1:]
                l = torch.relu(0.1 - sim_x_y + sim_x_x)
                loss_sim += sum(l)

            loss = loss_cox + loss_sim
